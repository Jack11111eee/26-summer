...
5. 普通非 gate item 按之前建议的观察集合定义进行补算：有效观察集合只包括 `SCORED`，拒答是否加入观察集合需要按特殊观察规则继续实现，但不得把缺证据、无效、模型不确定等混入。
6. 没有任何有效观察时不能补算。
...
16. 人工覆盖原始评分时需要二次人工确认；题库/模型失效或不可用时也需要二次人工确认。
...
4. 接受单进程 `BackgroundTasks`，后续有并发需求再优化。
5. 同时要求进程重启后任务可恢复。
6. 报告/任务重试覆盖原任务，不允许部分题库或部分评分直接发布。
7. 任务失败直接向用户显示可理解的失败状态。
8. 接受统一 pytest/CI 入口、eval 独立数据库和端到端流程测试。
9. 一致性测试、虚拟候选人测试和 bad-case 候选的阈值/基准/实现方式按第八轮建议执行；mock 回归不能替代真实 LLM 质量验证。

### 存在的重大矛盾，需继续对齐

“接受单进程 BackgroundTasks”与“要求进程重启后任务可恢复”不能仅靠当前 FastAPI 内存任务同时满足。至少还需要选择一种补充机制：

1. 接受单进程演示模式下任务可能丢失，将“重启恢复”降为后续增强；
2. 保留单进程执行器，但增加持久化任务表、启动扫描和可恢复任务状态；
3. 采用外部持久化队列/worker（这会超出当前单机演示范围）。

这是本轮唯一需要用户进一步明确的重大架构矛盾之一。在没有确认前，只能记录为“BackgroundTasks 可作为演示执行方式，但重启恢复机制未收敛”。
...
根据本轮回复，绝大多数原未决事项已经有了方向。当前仍需要用户明确或下一轮进一步细化的重点，收缩为：

1. **BackgroundTasks 与重启恢复的矛盾：** 单进程演示是否增加最小持久化任务状态/启动恢复，还是明确把恢复延期。
2. **难度到 1–5 的具体映射：** `observable_level_max`、required/target level、hard 最高等级证据如何形成确定表格。
3. **拒答特殊观察值的数学聚合：** 拒答 0 与后续有效回答同时存在时，具体如何进入 item 聚合。
4. **综合题与普通题的最终合并：** 各自有独立 item 分数时，如何避免重复证据和重复测量。
5. **事件表最终契约：** 最终字段、枚举、回放不一致修复、trace link 结构。
6. **计时与恢复的边界：** 有效计时区间、断线/无活动/abandoned 的精确规则。
7. **报告和人工复核发布：** 人工 override 后是否自动重新生成报告、谁执行最终发布。
8. **文档路径生效规则：** 新总设计文档放入 `design/final-design/` 后如何正式取代旧 SSOT。
