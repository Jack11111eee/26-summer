...
1. **orphan 列表字段口径与 status 过滤**
   ...
   - Recommendation: 取现有 `positions.py` 实现为准（字段子集 + `status != 'failed'`，与 `get_todos` 计数口径一致、前端已只消费 job_title/company/source_type/created_at/jd_id），D-52 的「字段同 list_jds」理解为「风格同 list_jds」而非逐字段。规划时在计划里写明口径并在 checker 前闭环。
...
2. **admin todos 失败明细的字段结构**
   ...
   - Recommendation: 后端做「计数 + 失败明细」（`question_bank_not_ready` 保持 int，新增 `question_bank_failed` 明细列表）；前端展示是否纳入本 phase 由规划时定——D-51 字面「前端零破坏」倾向不强制前端改动，但需与「生成失败对管理员可见」的验收口径对齐。
