...
8. 中文学术级唯一线索 = **Chinese-SkillSpan**（arXiv 2604.23009：中文招聘广告 span 级技能标注、2014–2025、4 平台，含 IID/OOD 划分与 scorer）；数据页在 Google Sites，沙箱不通，待**本机核两点：许可是否允许商用/课程使用、是否随包提供正文全文（而非仅标注）**。若均满足，是最贴合「中文 JD + 能力画像」的学术级资产。
...
- [ ]（可选 · 中文学术线索）本机核 Chinese-SkillSpan：许可是否允许课程使用 + 是否随包含正文全文——若可，是最贴合「中文 JD + 能力」的学术级资产
