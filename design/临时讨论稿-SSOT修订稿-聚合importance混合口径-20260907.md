...
- [ 同意 ] 改动点 1（§8.1 工序⑤行）：同意 / 修改意见：＿＿＿
- [ 同意 ] 改动点 2（§6 常量）：同意 / 修改意见：＿＿＿
- [ 同意 ] 改动点 3（§14 日志行）：同意 / 修改意见：＿＿＿
- [ 不放宽 ] 开放点 1（occ 门槛自适应）：不放宽 / 放宽
- [ 排除 ] 开放点 2（软技能 allowed）：允许 / 排除
- [ 展示 ] 开放点 3（证据面板三数）：展示 / 不展示
- [ 是 ] 确认后授权写入 SSOT（原子 commit）：是 / 否
