...
以下问题是本轮最需要优先回答的部分。没有确认前不进入正式设计和代码实施。

## A. 权重与配额

1. `hard_skill : soft_skill = 7 : 3` 是否确认定义为普通能力项最终 `item.weight` 的大类总权重比例，而不是题量比例？
2. 当只有 hard 或只有 soft 时，是否按本文公式将存在的大类重新归一到 1.00？
3. `required / preferred / plus` 是否只影响 item 原始重要性、配额和覆盖优先级，不再额外乘最终分？
4. `N` 是否作为岗位级策略配置的目标主问题数量，而不是从 7:3 自动唯一决定？
5. 静态难度是否只参与难度路径/题量结构，不参与最终 item.weight？

## B. 题目关系与综合题

6. 是否确认普通题单主评分 item、综合题单独题库且只在最后阶段出现？
7. 综合题默认每 session 最多一题，必要时最多两题，是否接受？
8. 是否确认 `qualification`/`experience` 保留为模型可追溯项，但通过 `measurement_mode=form` 排除普通对话选择？
9. 是否确认暂不建立 `question_bank_relation` 表，只保留扩展记录/接口？

## C. 难度与证据

10. 是否接受难度采用“能力等级证据上限”语义，而不是难度直接加分？
11. 是否接受 `easy → medium` 一次充分证据、`medium → hard` 一次充分证据加稳定路径记录？
12. 是否接受降级后恢复需要两次连续充分证据，或一次强证据？
13. `evidence_sufficient` 是否采用覆盖 target、具体可归因行为、原文可定位、无异常状态等条件？
14. `stable_evidence` 是否采用两次独立有效观察或一次高难度强证据？
15. “有机会达到高等级”是否采用岗位/rubric允许高等级与低/中难度达到进入条件的交集？
16. 每个满足条件的 item 至少尝试一题最高难度题，是否接受？

## D. required 例外、缺失与总分

17. required 刚性例外是否只增加测量机会，不改变 `item.weight`？
18. 是否接受 `N_actual = N_plan + required_exception_count` 的记录方式？
19. 拒答是否确认 `score_value=0 + score_state=REFUSED`，且参与该普通项聚合？
20. 缺失项比例补算是否只针对普通非 gate 项并标记 `IMPUTED`？
21. required 缺失时，是否接受“可生成带警告临时报告，可供人工排序参考，但不得仅凭其自动得出最终录用结论”的解释？
22. 缺失 required 是否必须触发补测/人工复核，即使临时报告已经生成？
23. `normalized_item_score=(score_final-1)/4` 是否作为 1–5 到 0–1 的基础归一化？

## E. 状态、计时与上下文

24. 是否确认时间不参与选题过滤，只由全场 40 分钟和单题 20 分钟计时器终止交互？
25. 是否确认全局追问不设独立上限，但每题最多两次且受计时器熔断？
26. 是否接受 P-refine、历史滑窗、结构化导航摘要三层分工？
27. 是否确认新增一张 `assessment_state_event` 表，并与当前状态字段在同一事务更新？
28. 是否确认暂不设置 Token 数值上限，但保留接口？

## F. Prompt

29. 下一轮是否按第 13 节的 P1–P26 清单逐项确定“需要 LLM / 代码替代 / 暂不做”？
30. Prompt 是否由用户先拟定初稿，我再负责 schema、边界、测试和与状态机的对齐？
31. 是否将 `evidence_sufficient`、`stable_evidence` 和 `answer_state` 的输出作为 Prompt 对齐的优先场景？
