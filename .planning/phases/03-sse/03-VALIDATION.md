...
| 03-01-02 | 01 | 1 | REF-2.4 | T-03-03 | form_instance DDL/迁移幂等；gate 九列；四步放宽（RENAME）；gate 行可插 NULL 列（裸连接 FK-off 烟测——parent-row 免依赖）；无新 DB CHECK | unit | `python -c` DDL 烟测（Task 2 verify——裸连接免父行）+ `pytest test_phase3_forms.py -v` | ❌ W0 | ⬜ pending |
