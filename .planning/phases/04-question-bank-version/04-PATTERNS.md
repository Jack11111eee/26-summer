...
2. 字段/status 口径：**【已裁定 [04-009] 选项 B 现有实现】**——字段子集 `jd_id/job_title/company/source_type/status/created_at` + `AND status != 'failed'`（与 `get_todos` 的 orphan 计数口径一致）。
...
1. **orphan 列表字段/status 口径**：【已裁定 [04-009] 选项 B】现有 `positions.py:87-95` 实现（字段子集 + `AND status != 'failed'`，与 `get_todos` 计数一致）。
2. **todos 失败明细结构**：已落定——`question_bank_not_ready`（int）保持 + 新增 `question_bank_failed`（list）。实现细节（是否去重 position、过滤空 error_msg）由执行时定。
3. **前端是否加「题库失败」卡**：【已裁定 [04-010] 选项 A 前端零破坏】本 phase 不做前端改动。
